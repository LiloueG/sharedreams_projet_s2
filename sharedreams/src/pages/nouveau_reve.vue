...
const forbiddenWords = ["putain", "merde", "pute", "con", "baise", "baisé", "baiser", "enculé", "enculer"];
...
const handleValidation = async () => {
    ...
    } else if (containsForbiddenWords(dream.value)) {
        alert("Votre message contient des mots non autorisés.");
    } else {
...
<template>
    <main class="px-4 py-12 min-h-screen">
        ...
        <div class="flex justify-center gap-4 mt-16">
            <button @click="layout = 'dream'" class=" border-2 px-8 py-2 rounded-full font-Marigny font-semibold"
                :class="{ 'border-orange-400 border-2 bg-orange-400': layout == 'dream' }">Rêve</button>
            <button @click="layout = 'nightmare'" class="border-2 px-8 py-2 rounded-full font-Marigny font-semibold"
                :class="{ 'border-orange-400 border-2 bg-orange-400': layout == 'nightmare' }">Cauchemar</button>
        </div>
        ...
        <form>
            <div class="mt-10">
                <textarea type="text" id="name" v-model="dream"
                    class="w-full h-64 font-light placeholder:text-white px-4 py-2 border-white border-2 rounded-md bg-slate-200/50 "
                    placeholder="Ecrivez votre message" />
            </div>
        </form>
        <div class="flex justify-end mt-2">
            <button @click="handleValidation()"
                class="bg-orange-400 border-2 border-orange-400 px-8 py-2 rounded-full font-Marigny font-semibold self-end ">Publier</button>
        </div>
    </main>
</template>